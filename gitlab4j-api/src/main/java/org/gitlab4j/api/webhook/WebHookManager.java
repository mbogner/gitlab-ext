...
public class WebHookManager implements HookManager {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(WebHookManager.class.getName());
    ...
    public Event handleRequest(HttpServletRequest request) throws GitLabApiException {
        ...
        try {
            ...
            if (LOGGER.isTraceEnabled()) {
                LOGGER.trace(HttpRequestUtils.getShortRequestDump(eventName + " webhook", true, request));
                String postData = HttpRequestUtils.getPostDataAsString(request);
                LOGGER.trace("Raw POST data:\n" + postData);
                event = jacksonJson.unmarshal(Event.class, postData);
                LOGGER.trace(event.getObjectKind() + " event:\n" + jacksonJson.marshal(event) + "\n");
            } else {
